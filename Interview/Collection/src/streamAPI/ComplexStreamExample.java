...
class Employee1 {
	...
	public String toString() {
		return String.format("Employee{id=%d, name='%s', department='%s', salary=%.2f, age=%d}", id, name, department,
				salary, age);
	}
}
...
public class ComplexStreamExample {
	public static void main(String[] args) {
		List<Employee1> employees = Arrays.asList(new Employee1(1, "Alice", "HR", 50000, 30),
				new Employee1(2, "Bob", "IT", 70000, 25), new Employee1(3, "Charlie", "Finance", 80000, 40),
				new Employee1(4, "Diana", "IT", 75000, 28), new Employee1(5, "Edward", "Finance", 90000, 50),
				new Employee1(6, "Fiona", "HR", 60000, 35));

		// 1. Group employees by department
		Map<String, List<Employee1>> employeesByDept = employees.stream().collect(groupingBy(Employee1::getDepartment));
		...
		List<String> sortedNamesByAge = employees.stream().sorted(Comparator.comparingInt(Employee1::getAge))
				.map(Employee1::getName).collect(toList());
		...
		Map<Integer, List<Employee1>> employeesByAge = employees.stream().filter(emp -> emp.getAge() > 30)
		...
		double totalItSalary = employees.stream().filter(emp -> "IT".equals(emp.getDepartment()))
				.mapToDouble(Employee1::getSalary).sum();
		System.out.println("\nTotal salary of IT department: " + totalItSalary);

		// 7. Check if all employees are older than 20
		boolean allOlderThan20 = employees.stream().allMatch(emp -> emp.getAge() > 20);
